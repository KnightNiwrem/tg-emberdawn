/** Source navigation retains the detail scene's item and return context. */
...
export function sourcesAction(p: PlayerState, cb: Cb & { v: 'sources' }): { toast?: string } {
  ...
  if (cb.a === 'bk') {
    if (!s.arg3?.startsWith('sources:')) return {};
    delete s.arg3;
  } else {
    s.arg3 = `sources:${cb.arg}`;
  }
  return {};
}
