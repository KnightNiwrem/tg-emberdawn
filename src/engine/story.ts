/**
 * Declarative story transactions: effects run in authored order on a draft,
 * through the quest/inventory authorities. Only a successful bundle commits
 * and records its receipt; refusals leave the live player unchanged.
 * Readiness describes the final draft, and terminal quest outcomes are monotonic.
 * See .agents/skills/emberdawn-story-and-quests/SKILL.md for the full contract.
 */
...
/** Run effects against the projected draft in authored order; stop at the first
 * refusal. Validation and application share this run. Reconcile notices only
 * after all effects succeed, preserving immediate quest-readiness transitions. */
function runStoryBundle(
  ...
  // Capture readiness before syncAvailability's internal refresh can consume
  // the active→turnIn transition without reporting it.
  result.readyQuests.push(...refreshQuestProgress(draft));
  syncAvailability(draft);
  // Later effects can revoke earlier readiness; announce each final ready quest once.
  result.readyQuests = [...new Set(result.readyQuests)].filter(
    (id) => draft.quests[id]?.status === 'turnIn',
  );
  // Only quests already started at entry earn a cancellation notice.
  result.lines.push(
...
/** Choice applications supply an id; line-entry applications use dialogue + node. */
function receiptKey(ctx: StoryContext): string {
  return ctx.applicationId ?? `line:${ctx.dialogueId}:${ctx.nodeId}`;
}

/** Commit the completed draft and its receipt together; refusals never reach here. */
...
/** Return a refusal or undefined using the same draft run as application.
 * Existing receipts validate as no-ops after the journey/dungeon guards. */
...
/** Commit all effects and a receipt, or throw without changing the live player.
 * An existing receipt returns an empty result after journey/dungeon guards. */
...
/** The caller supplies a choice; the live scene and content supply its authority. */
...
/** Resolve the choice from the live scene and recheck NPC presence, availability,
 * and staged confirmation. Commit effects and the decision ledger atomically.
 * Receipted retries retain navigation but repeat no story mutations or notices.
 * The locked callback router owns message identity and revision checks. */
