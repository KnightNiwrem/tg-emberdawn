...
Authoritative code and tests: `src/engine/quests.ts`, `src/content/quests.ts`,
`src/content/types.ts`, `src/engine/types.ts`, `src/engine/world.ts`, `src/handlers/hub.ts`,
`tests/quest_authority_test.ts`, `tests/quest_ready_notice_test.ts`, `tests/quest_copy_test.ts`,
`tests/questlog_test.ts`.

For offer/turn-in dialogue wiring or conversation objectives, also read
[Dialogue and NPC navigation](dialogue-and-npcs.md). For conditions, terminal outcomes, or quest
mutations within a StoryEffect bundle, read the relevant sections of
[Choices and story transactions](choices-and-transactions.md).
...
- Objectives tick through engine hooks: `onKill`, `onZoneEnter`, `onDungeonClear`; collect
  objectives read the bag live. Conversation progression is explicit story events (see
  [NPC topic menus](dialogue-and-npcs.md#npc-topic-menus)).
