...
Deno.test('quest log pages side quests — the 9th live quest is reachable (#21)', async () => {
  ...
  assert(page1.includes('q:pg:2'), 'page 1 offers Next');
  assert(page2.includes('q:q:sq_null'), 'the 17th quest lives on the last page');
  assert(page2.includes('q:pg:1'), 'the last page offers Prev');
  assert(!page2.includes('q:pg:3'), 'the last page has no Next');
  ...
  await handleCallback(fakeCtx(940, 700, withRev(cur.uiRev ?? 0, 'q:a:sq_scarabs')), store);
  cur = (await store.get(940))!;
  assertEquals(
    cur.quests['sq_scarabs']?.status,
    'available',
