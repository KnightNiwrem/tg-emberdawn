...
function battleFor(id: number, enemyId = 'e_rat'): { p: PlayerState; b: BattleState } {
  ...
  const b = startBattle(enemyId, ORIGIN, { player: p, rng: seeded(id) })!.battle;
  p.battle = b;
  return { p, b };
}
...
Deno.test('shields: bypassShield lands on HP and leaves the ward untouched (#79)', () => {
  ...
  for (let seed = 1; seed <= 120 && !observed; seed++) {
    ...
    p.level = 45;
    const b = startBattle('e_aldric', BOSS_ORIGIN, { player: p, rng })!.battle;
    p.battle = b;
...
Deno.test('shields: enemy wards absorb, expire and announce (#79)', () => {
  ...
  p.level = 17;
  const b = startBattle('e_sentinel', ORIGIN, { player: p, rng: seeded(30) })!.battle;
  p.battle = b;
  let seen = false;
...
Deno.test('shields: Aegis of Dawn grants a real ward through the resolver (#79)', () => {
  ...
  p.mp = 999;
  const b = startBattle('e_rat', ORIGIN, { player: p, rng: seeded(40) })!.battle;
  b.enemy.hp = 99999;
  b.enemy.maxHp = 99999;
  p.battle = b;
  stunEnemy(b);
