...
Deno.test('choices: irreversible selection stages confirmation; open/back mutate nothing', () => {
  ...
  const panel = JSON.stringify(renderDialogue(player));
...
Deno.test('choices: confirmation applies effects exactly once, atomically (#126, #132)', () => {
  const player = ferryHero(1403);
  openChoice(player);
  dialogueAction(player, { v: 'dlg', a: 'ch', arg: 'promise' }); // stage
  dialogueAction(player, { v: 'dlg', a: 'cf', arg: 'promise' }); // confirm
  assertEquals(player.decisions['ferry_shrine_pledge']?.choiceId, 'promise');
  assertEquals(player.storyEvents, ['shrine_allegiance_chosen'], 'shared parent progress emitted');
...
Deno.test('choices: the decline branch starts the other route and locks the first', () => {
  const decliningPlayer = ferryHero(1404);
  const promisingPlayer = ferryHero(1405);
  openChoice(decliningPlayer);
  ...
  assertEquals(
    decliningPlayer.storyEvents,
    promisingPlayer.storyEvents,
    'both choices share the parent-progress event',
  );
  // Each route starts its own follow-up quest and locks the other (#132).
  assertEquals(decliningPlayer.quests['sq_ledger_debt']?.status, 'active');
  assertEquals(decliningPlayer.questOutcomes['sq_shrine_pact']?.kind, 'locked');
  assertEquals(promisingPlayer.quests['sq_shrine_pact']?.status, 'active');
  assertEquals(promisingPlayer.questOutcomes['sq_ledger_debt']?.kind, 'locked');
});
...
Deno.test('choices: full router — forged and mismatched cf callbacks are harmless refusals (#136)', async () => {
  ...
  let tapResult = await tap('dlg:cf:decline');
  assert(
    tapResult.toasts.some((toast) => toast?.includes('moved on')),
    `refusal toast: ${tapResult.toasts}`,
  );
  assertEquals(
    Object.keys(tapResult.cur.decisions).length,
    0,
  ...
  const stale = (tapResult.cur.uiRev ?? 1) - 1;
  tapResult = await tap('dlg:ch:promise', stale);
  assert(
  ...
  tapResult = await tap('dlg:cf:promise', (tapResult.cur.uiRev ?? 1) - 1);
  assert(
    tapResult.toasts.some((toast) => toast?.includes('stale')),
    `staleness toast: ${tapResult.toasts}`,
  );
  ...
  tapResult = await tap('dlg:cf:decline');
  assert(
    tapResult.toasts.some((toast) => toast?.includes('moved on')),
    `refusal toast: ${tapResult.toasts}`,
  );
  ...
  tapResult = await tap('dlg:cf:promise');
  assertEquals(Object.keys(tapResult.cur.decisions).length, 1);
  assertEquals(tapResult.cur.storyReceipts.length, 1, 'no second receipt, no double apply');
});
...
Deno.test('choices: scene persists through rerender and /start (#126)', () => {
  ...
  dialogueAction(player, { v: 'dlg', a: 'cf', arg: 'promise' });
  const n4 = JSON.stringify(renderDialogue(player));
  assertEquals(n4, JSON.stringify(renderDialogue(player)));
});
