# Combat & Mechanics Guide — Emberdawn
...
$$\text{dodgeChance} = \text{clamp}(0.02 + (\text{spd} - \text{enemySpd}) \times 0.002, 0.02, 0.20)$$
...
- **Scripted Specials:** An enemy move with `special.every = N` triggers on the enemy's $N$-th
  actual action (e.g., actions 3, 6, 9...). Turns spent stunned increment the counter (time
  elapses), but do not execute the special during the stun.
...
- **Phoenix Cinder:** Automatically revives the hero once per fight at 40% HP upon fatal damage
  (`b.phoenixUsed`); it cannot be triggered manually from inventory.
