# Persistence & Schema Guide — Emberdawn
...
- Runtime-only battle state (e.g. temporary buffs, transient turn history) lives on `BattleState`,
  not on `PlayerState`.
- Narrative state fields:
  ...
  - `p.questOutcomes`: Permanent resolution map (`completed`, `locked`, `failed`).
  - `p.storyReceipts`: Set of one-shot keys suppressing replayed choice or line mutations.
...
The load-time validation gate `assertSupportedSaveVersion()` is non-mutating:
